...
- [2 国内外研究现状及分析](#2-国内外研究现状及分析)
  - [2.1 国内外研究现状](#21-国内外研究现状)
    - [2.1.1 RISC-V 指令集架构](#211-risc-v-指令集架构)
    - [2.1.2 乱序执行技术](#212-乱序执行技术)
    - [2.1.3 开源处理器核](#213-开源处理器核)
    - [2.1.4 硬件描述语言与验证方法](#214-硬件描述语言与验证方法)
- [3 主要研究内容及研究方案](#3-主要研究内容及研究方案)
  - [3.1 研究内容](#31-研究内容)
    - [3.1.1 多周期基础处理器的设计与实现](#311-多周期基础处理器的设计与实现)
    - [3.1.2 乱序执行微架构的设计与实现](#312-乱序执行微架构的设计与实现)
    - [3.1.3 系统软件适配与操作系统引导](#313-系统软件适配与操作系统引导)
  - [3.2 研究方案](#32-研究方案)
    - [3.2.1 基础处理器搭建与验证](#321-基础处理器搭建与验证)
    - [3.2.2 乱序执行核心模块设计](#322-乱序执行核心模块设计)
    - [3.2.3 系统集成与操作系统启动](#323-系统集成与操作系统启动)
- [4 进度安排及预期目标](#4-进度安排及预期目标)
  - [4.1 进度安排与预期目标](#41-进度安排与预期目标)
- [5 已具备和所需的条件和经费](#5-已具备和所需的条件和经费)
  - [5.1 已具备的条件](#51-已具备的条件)
  - [5.2 所需条件和经费](#52-所需条件和经费)
- [6 预计困难及解决方案](#6-预计困难及解决方案)
  - [6.1 预计困难与解决方案](#61-预计困难与解决方案)
    - [6.1.1 乱序执行逻辑复杂度高](#611-乱序执行逻辑复杂度高)
    - [6.1.2 验证覆盖率难以保障](#612-验证覆盖率难以保障)
    - [6.1.3 操作系统引导涉及特权级适配](#613-操作系统引导涉及特权级适配)
- [参考文献](#参考文献)
...
得益于 RISC-V 的开放性，学习者可以在没有商业授权限制的条件下，亲手实现一颗从前端取指到后端提交的完整处理器核，这在传统的 x86 或 ARM 体系下几乎不可能做到。对于计算机科学与技术专业的本科生而言，亲自设计并实现一个支持乱序执行的 RISC-V 处理器，是将课堂上学到的体系结构理论知识付诸实践的绝佳途径，也是笔者本科阶段一直以来的学习心愿。

### 1.2 研究的目的和意义

本课题旨在依据 RISC-V 指令集规范，设计并实现一个支持乱序执行的处理器。具体而言，通过引入**寄存器重命名**（Register Renaming）[5]、**发射队列**（Issue Queue）、**乱序执行单元**和**重排序缓冲区**（Reorder Buffer, ROB）[6]等核心微架构模块，解决指令间的数据相关（RAW/WAR/WAW）问题，实现指令的乱序发射、乱序执行与顺序提交。

本课题的研究意义主要体现在以下几个方面：

**深入理解体系结构原理。** 处理器微架构设计是计算机体系结构课程的核心内容，但课堂教学往往止步于原理讲解和简单的流水线实验。通过亲手从零设计一个乱序处理器，能够将流水线设计、动态调度、分支预测[7]、存储层次等书本知识转化为可运行的硬件实现，真正做到"知其然，更知其所以然"。这一过程本身就是对本科阶段所学知识的一次综合性检验与深化。

**获得完整的工程实践经验。** 从 RTL 编码、功能仿真、FPGA 原型验证到操作系统启动，本课题覆盖了处理器设计的完整工程流程。这种"全栈"的实践经历对于培养系统级的工程思维和问题排查能力十分有益，也为日后在体系结构方向的深入学习打下扎实基础。

**借助开源生态降低学习门槛。** RISC-V 的开放性以及 BOOM[8]、香山[9]等成熟开源项目的存在，使得本科生也有条件参考工业级设计进行学习和实践。本课题的设计过程和产出（代码、文档、验证框架等），也可为后续有志于体系结构方向的同学提供一份可复现的学习案例。

**实现系统级验证闭环。** 课题不仅关注微架构设计本身，还计划通过运行 xv6[10] 等教学操作系统以及尝试引导 Linux 内核启动来验证处理器的功能完整性和系统兼容性。能够在自己设计的处理器上跑起操作系统，是检验设计正确性最直观也最令人兴奋的方式。
