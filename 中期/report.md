...
- **ifu.valid.wait**：等待取指单元（IFU）返回有效指令。取指完成后根据指令类型分流——若为访存指令，进入 mem.ready.wait；若为非访存指令，且未出现译码异常，则进入 WB，若发现译码异常，则进入 exception；
...
参考模型采用 Spike[19]（RISC-V 官方指令集模拟器）。如 2.2 节所述，本课题 fork 了 Spike 并修改部分源码以适配当前处理器的 M-mode 特权级实现，将其以静态库形式融入 Rust 仿真平台中。同时集成了 Capstone 反汇编引擎，支持在仿真过程中对机器码进行实时反汇编，辅助指令流分析。DiffTest 框架在处理器开发过程中持续发挥作用，已帮助定位并修复了多个微架构实现中的功能缺陷。

### 2.6.2 仿真平台的技术选型

仿真平台选择 Rust 语言开发（详见 2.2 节），主要基于以下考量：相较于 C++，Rust 具有更现代的语言设计和更优雅的抽象能力——其代数数据类型（enum）与模式匹配使状态机和协议解析的表达更加简洁清晰，trait 系统提供了比 C++ 模板更直观的泛型约束机制，这些特性使仿真平台中涉及多种 CPU 模型抽象、DPI-C 回调分发等复杂逻辑的代码更易编写和维护；Rust 的构建工具 Cargo 和 `build.rs` 机制简化了跨语言编译和链接的管理，能够方便地将 Spike、Capstone、Verilator 等 C/C++ 外部库统一集成；Rust 的 FFI 能力使其能够与 C/C++ 库无缝互操作。
...
自 2025 年 12 月 27 日项目启动至今，本课题已积累了较为可观的工程量。表 5-1 汇总了各子项目的代码统计数据（使用 cloc 工具统计，仅计入有效代码行，不含空行与注释）。

| 子项目           | 主要内容                                | 文件数  | 有效代码行 | 主要语言           |
| ---------------- | --------------------------------------- | ------- | ---------- | ------------------ |
| 处理器核心与 SoC | CPU 核心、SoC 总线与外设控制器          | 53      | 5,573      | Scala (Chisel)     |
| 仿真平台         | DiffTest 框架、SDB 调试器、追踪工具     | 26      | 2,256      | Rust               |
| 仿真模型与桥接   | Verilator/Spike/NVBoard FFI 封装        | 25      | 2,251      | Verilog/C++        |
| SPI/QSPI 控制器  | PSRAM QSPI Master、SPI Master、仿真模型 | 20      | 2,177      | Scala/C++/Verilog  |
| SDRAM 控制器     | SDRAM AXI4 控制器、仿真模型、UVM 测试   | 31      | 2,509      | Scala/C++/C Header |
| 裸机运行时       | AM 硬件抽象层、klib 运行时库            | 46      | 5,268      | C/Assembly         |
| **合计**         |                                         | **201** | **20,034** |                    |

其中，处理器核心与 SoC 的硬件设计代码（Scala/Chisel）约 7,284 行，仿真与验证基础设施（Rust + C/C++）约 4,507 行，存储控制器及仿真模型约 4,686 行，裸机运行时支持约 5,268 行。各子项目的详细统计如下。

**处理器核心、SoC 与仿真平台**（共 10,080 行有效代码）：

```
Language                         files          blank        comment           code
-----------------------------------------------------------------------------------
Scala                               53            958            643           5573
Rust                                26            283             33           2256
Verilog-SystemVerilog               19            299           1507           2006
C++                                  3             31             13            167
C/C++ Header                         3             17              5             78
-----------------------------------------------------------------------------------
SUM:                               104           1588           2201          10080
```

**裸机运行时支持**（AM 硬件抽象层与 klib 运行时库，共 5,268 行有效代码）：

```
Language                     files          blank        comment           code
-------------------------------------------------------------------------------
C                               30            656            488           2684
C/C++ Header                    11            255            245           2295
Assembly                         5             49             96            153
D                               24              0              0            125
make                             2              4              2             11
-------------------------------------------------------------------------------
SUM:                            72            964            831           5268
...
**SPI/QSPI 控制器与 PSRAM 仿真模型**（共 2,177 行有效代码）：
...
Scala                                7            172            214            896
C++                                  4            133            191            664
Verilog-SystemVerilog                9            131            313            617
-----------------------------------------------------------------------------------
SUM:                                20            436            718           2177
```

**SDRAM 控制器与仿真模型**（共 2,509 行有效代码）：

```
Language                         files          blank        comment           code
-----------------------------------------------------------------------------------
C/C++ Header                        14            185            151            896
Scala                                9            128             39            815
C++                                  6            166            158            756
Verilog-SystemVerilog                1              1              0             27
C                                    1              4              0             15
-----------------------------------------------------------------------------------
SUM:                                31            484            348           2509
```

以上数据表明，自项目启动以来的约 10 周时间内，本课题已完成超过 2 万行有效代码的编写与调试工作，涵盖硬件设计、仿真验证、存储控制器、运行时支持等多个层面，为后续的乱序流水线设计和特权架构扩展奠定了坚实的工程基础。
